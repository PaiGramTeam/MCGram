...
class WishLogPlugin(Plugin.Conversation):
    ...
    async def _refresh_user_data(
        ...
        try:
            ...
            return "凌阳没有找到您所绑定的账号信息，请先私聊凌阳绑定账号"
    ...
    async def import_data_from_message(self, update: Update, _: CallbackContext) -> int:
        ...
        reply = await message.reply_text("小凌阳正在从服务器获取数据，请稍后", reply_markup=ReplyKeyboardRemove())
        await message.reply_chat_action(ChatAction.TYPING)
    ...
    async def command_start_export(self, update: Update, context: CallbackContext) -> None:
        ...
        try:
            ...
            await message.reply_text(
                "凌阳没有找到你的唤取记录，快来私聊凌阳导入吧~", reply_markup=InlineKeyboardMarkup(buttons)
            )
        except GachaLogAccountNotFound:
    ...
    async def wish_log_pool_send(self, user_id: int, pool_type: "MCBannerType", message: "Message"):
        await message.reply_chat_action(ChatAction.TYPING)
