...
from kuronet.errors import DataNotPublic, BadRequest as SimnetBadRequest
...
class DailyNoteTasksPlugin(Plugin.Conversation):
    ...
    async def command_start(self, update: Update, _: CallbackContext) -> int:
        ...
        url = f"{config.pass_challenge_user_web}/tasks3?command=tasks&bot_data={note_user.web_config}"
    ...
    async def check_genshin_user(
        ...
        try:
            async with self.helper.genshin(user_id, player_id=uid, offset=offset) as client:
                client: "MCClient"
                if request_note:
                    await client.get_mc_notes_widget()
                return client.player_id
        except ValueError:
            return "Cookies 缺少 stoken ，请尝试重新绑定账号。"
        except DataNotPublic:
            return "查询失败惹，可能是便签功能被禁用了？请尝试通过库街区获取一次便签信息后重试。"
